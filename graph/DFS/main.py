...
def dfs(graph, start_node):
    ...
    while stack:
        current_station = stack.pop()  # 큐의 가장 앞 데이터를 갖고 온다
        current_station.visited = 2
        
        for i in range(len(current_station.adjacent_stations)):
            if current_station.adjacent_stations[i].visited == 0:
                current_station.adjacent_stations[i].visited = 1
                stack.append(current_station.adjacent_stations[i])
