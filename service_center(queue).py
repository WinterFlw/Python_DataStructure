...
class CustomerServiceCenter:
    ...
    def process_complaint(self):
        ...
        # 여기에 코드를 작성하세요
        if self.queue:
            current_complaint = self.queue.pop()
            print(f"{current_complaint.name}님의 {current_complaint.content} 문의 내용 접수 되었습니다. 담당자가 배정되면 {current_complaint.email}로 연락드리겠습니다!")
        else:
    ...
    def add_complaint(self, name, email, content):
        ...
        self.queue.appendleft(new_complaint)
# 고객 문의 센터 인스턴스 생성
